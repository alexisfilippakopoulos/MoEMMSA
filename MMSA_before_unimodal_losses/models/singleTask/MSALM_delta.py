...
class SparseMMTokenRouter(nn.Module):
    def __init__(self, config, n_experts, top_k, d_av):
        ...
        self.d_av = d_av                   # e.g., 30 (input dim of Z_a, Z_v)

        # We concatenate the specific token embedding with the global context
    ...
    def forward(self, x_f, z_a, z_v, z_av):
        """
        x_f  : [B, T_f, 768] -> The learnable fusion tokens (NOT pooled)
        z_a  : [B, T_a, 30]
        z_v  : [B, T_v, 30]
        z_av : [B, T_av, 30]
        """
        ...
        # 1. Create Global Context Summary (Pool the modalities)
        # We pool these because x_f tokens don't align 1-to-1 with audio/visual frames
        z_a_pool  = z_a.mean(dim=1)    # [B, 30]
        z_v_pool  = z_v.mean(dim=1)    # [B, 30]
        z_av_pool = z_av.mean(dim=1)   # [B, 30]
        
        # Combine contexts: [B, 90]
        context_summary = torch.cat([z_a_pool, z_v_pool, z_av_pool], dim=-1)

        # 2. Broadcast context to every fusion token
        # Expand [B, 90] -> [B, T_f, 90]
        context_expanded = context_summary.unsqueeze(1).expand(-1, T_f, -1)

        # 3. Create Router Input (Token + Context)
        # [B, T_f, 768] cat [B, T_f, 90] -> [B, T_f, 858]
        ...
        # Select Top-K for each token
        top_k_weights, top_k_indices = torch.topk(all_weights, self.top_k, dim=-1)

        # Re-normalize the selected weights
...
class MMSparseRouter(nn.Module):
    def __init__(self, n_experts, top_k, d_av, d_model):
        super().__init__()
        self.n_experts = n_experts
        self.top_k = top_k

        self.d_fusion = d_model      # 768
        self.d_av = d_av                   # 30

        # Total routing input dimension:
        # q_fusion (768) + z_a + z_v + z_av (3 × 30)
        self.d_total = self.d_fusion + 3 * self.d_av
    ...
    def forward(self, x_f, z_a, z_v, z_av):
        ...
        q_modality = torch.cat(
            [z_a_pool, z_v_pool, z_av_pool],
            dim=-1
        )  # [B, 90]
        ...
        # ---- Routing ----
        logits = self.router_weights(q_total)      # [B, n_experts]
        all_weights = F.softmax(logits, dim=-1)
...
class MoeMMBlock(nn.Module):
    ...
    def __init__(self, config, layer_idx, top_k):
        ...
        self.delta_k = nn.Linear(config.n_embd, config.n_embd, bias=False)
        self.delta_v = nn.Linear(config.n_embd, config.n_embd, bias=False)

        # beta gate: shared erase/write gate (scalar per layer)
        self.delta_beta = nn.Parameter(torch.zeros(1))
        self.delta_gate = nn.Sigmoid()
    ...
    def forward(self, x_q, z_a, z_v, z_av, x_prev=None, x_kv=None):
        ...
        # ---- DELTA-RESIDUAL FUSION TOKEN UPDATE ----
        delta_x_f = self.delta_dropout(delta_x_f)

        # value vector v_l (paper: v(X))
        v = self.delta_v(delta_x_f)  # [B, T_f, D]

        # direction vector k_l (paper: k(X))
        k = self.delta_k(x_q)        # [B, T_f, D]
        #k = F.normalize(k, dim=-1)  # ||k|| = 1 (required by theory)
        k = F.normalize(v.detach(), dim=-1)
        # projection of current state onto k
        # k^T X_l
        proj_old = (k * x_q).sum(dim=-1, keepdim=True)  # [B, T_f, 1]

        # projection of new value onto k
        # v_l^T
        proj_new = (k * v).sum(dim=-1, keepdim=True)    # [B, T_f, 1]

        # beta gate in [0, 1] (can later extend to [0, 2])
        beta = self.delta_gate(self.delta_beta)
        #router_conf = top_k_weights.mean(dim=-1, keepdim=True)
        #beta = self.delta_gate(self.delta_beta) * router_conf

        # Delta update: erase + write (single rank-1 operation)
        x_f_updated = x_q + beta * k * (proj_new - proj_old)
        #x_f_updated = x_q + delta_x_f + beta * k * (proj_new - proj_old)
        # ---- FEED-FORWARD ----
        if self.combine:
            x_comb = torch.cat((x_prev, x_f_updated), dim=1)
            x_f_updated = x_comb + self.gate_2(self.alpha_2) * self.mlp(self.ln_2(x_comb))
